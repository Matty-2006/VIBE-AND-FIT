...
function resolveAlt(value: ImageValue): string {
  if (value && typeof value === "object" && typeof value.alt === "string")
    return value.alt;
  return "";
}

function imageOf(item: ImageInput): Slide | null {
  if (item && typeof item === "object" && "image" in item) {
    const src = resolveSrc(item.image);
    if (!src) return null;
    return { src, alt: resolveAlt(item.image), offsetY: offsetOf(item) };
  }
  const src = resolveSrc(item as ImageValue);
  if (!src) return null;
  return { src, alt: resolveAlt(item as ImageValue), offsetY: 0 };
}
...
export default function SmoothScrollSlider({
  ...
  const source = useMemo<Slide[]>(() => {
    ...
    for (const item of images ?? []) {
      const slide = imageOf(item);
      if (slide) resolved.push(slide);
    }
  ...
  useEffect(() => {
    ...
    const onMove = (event: PointerEvent) => {
      ...
      lastX = event.clientX;
      target.current +=
        (input.current.flip ? dx : -dx) * input.current.dragMultiplier;
    };
